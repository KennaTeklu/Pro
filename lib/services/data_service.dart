...
class DataService {
  ...
  Exercise generateExercisePrescription(ExerciseDefinition def, {double phaseMultiplier = 1.0}) {
    ...
    final maturity = 1 - exp(-totalWorkouts / 100);
    ...
    // Trend analysis (linear regression on last 3 sessions)
    double? trendWeight;
    if (history.length >= 3) {
    ...
    if (bestWeight != null && bestWeight > 0) {
      ...
      baseWeight = (estimated1RM * 0.6 / 5).round() * 5.0;
    }
    ...
    if (def.muscles.isNotEmpty) {
      ...
      if (muscleDef != null) {
        final recoveryPercent = daysSince >= muscleDef.restDays ? 1.0 : daysSince / muscleDef.restDays;
        const k = 6.0;
  ...
  Map<String, dynamic> calculateLongevityScore() {
    ...
    if (age != null) {
      if (age < 30) {
        weights = {
          'gripStrength': 0.20, 'balance': 0.10, 'jointMobility': 0.15, 'posture': 0.15,
          'muscleBalance': 0.25, 'consistency': 0.10, 'trend': 0.05, 'variety': 0.0,
        };
      } else if (age < 50) {
        weights = {
          'gripStrength': 0.15, 'balance': 0.15, 'jointMobility': 0.25, 'posture': 0.15,
          'muscleBalance': 0.10, 'consistency': 0.10, 'trend': 0.05, 'variety': 0.05,
        };
      } else {
        weights = {
          'gripStrength': 0.10, 'balance': 0.25, 'jointMobility': 0.30, 'posture': 0.15,
          'muscleBalance': 0.05, 'consistency': 0.05, 'trend': 0.05, 'variety': 0.05,
        };
      }
    }
  ...
  double _consistencyScore() {
    ...
    final variance = weeklyCounts.map((v) => pow(v - mean, 2)).reduce((a, b) => a + b) / 8;
